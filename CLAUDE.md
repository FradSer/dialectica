...
- `workflow.py` — `Workflow` + `agent/parallel/pipeline/phase/log/Budget`: a composable multi-agent runtime (a Python port of Claude Code's `Workflow` tool surface) for **meta-task** orchestration (research/review/planning/design). Orchestration layer, NOT a self-contained-quality engine — the negative findings above stand. `agent()` uses ADK `output_schema` for structured output (reuses `gan_evaluator`'s fence/escape repair + re-ask); `parallel`/`pipeline` honor `DIALECTICA_MAX_CONCURRENCY`. Resume/journaling NOT in v1 (documented gap).
